...
For a power n sequanc, the constant difference is found after n-1 iterations. 
Once we find it, we can generate the next term of the squance we found on the n-2 iteration. 
We go up with that until we reached the first sequance and generate the next term for that sequance.
This new term is the BOP we are looking for a given PO(k,n).

Summerize all BOPs for PO(k,n) for k = 1 .. 10 to get the answer:
...
// recursice function to find BOP for given k
